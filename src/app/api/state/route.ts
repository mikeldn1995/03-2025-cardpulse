import { NextResponse } from "next/server"
import { db } from "@/db"
import { users, cards, monthlyRecords } from "@/db/schema"
import { eq } from "drizzle-orm"
import { getSession } from "@/lib/auth"

// GET: Load full user state from DB
export async function GET() {
  const result = await getSession()
  if (!result) return NextResponse.json({ error: "Unauthorized" }, { status: 401 })

  const userId = result.user.id
  const user = result.user
  const userCards = await db.select().from(cards).where(eq(cards.userId, userId))

  // Load monthly records for each card
  const cardsWithRecords = await Promise.all(
    userCards.map(async (card) => {
      const records = await db.select().from(monthlyRecords).where(eq(monthlyRecords.cardId, card.id))
      return {
        id: card.id,
        issuer: card.issuer,
        last4: card.last4,
        openingBalance: card.openingBalance,
        openingMonth: card.openingMonth,
        limit: card.creditLimit,
        aprRegular: card.aprRegular,
        aprPromo: card.aprPromo,
        promoUntil: card.promoUntil,
        dd: card.dd,
        ddAmount: card.ddAmount,
        paymentDay: card.paymentDay,
        statementDay: card.statementDay,
        source: card.source,
        tlAccountId: card.tlAccountId,
        minPaymentOverride: card.minPaymentOverride ?? null,
        monthlyRecords: records.map((r) => ({
          month: r.month,
          debits: r.debits,
          credits: r.credits,
          interest: r.interest,
          closingBalance: r.closingBalance,
          source: r.source,
        })),
      }
    })
  )

  return NextResponse.json({
    userName: user.name,
    userEmail: user.email,
    utilThreshold: user.utilThreshold,
    currency: user.currency,
    theme: user.theme,
    forecastMonthly: user.forecastMonthly,
    onboarded: user.onboarded,
    cards: cardsWithRecords,
  })
}

// PUT: Save full user state to DB
export async function PUT(req: Request) {
  const result = await getSession()
  if (!result) return NextResponse.json({ error: "Unauthorized" }, { status: 401 })

  const userId = result.user.id
  const body = await req.json()

  // Update user preferences
  await db.update(users).set({
    name: body.userName ?? result.user.name,
    currency: body.currency ?? result.user.currency,
    theme: body.theme ?? result.user.theme,
    utilThreshold: body.utilThreshold ?? result.user.utilThreshold,
    forecastMonthly: body.forecastMonthly ?? result.user.forecastMonthly,
    onboarded: body.onboarded ?? result.user.onboarded,
  }).where(eq(users.id, userId))

  // Sync cards
  if (body.cards) {
    const existingCards = await db.select().from(cards).where(eq(cards.userId, userId))
    const existingIds = new Set(existingCards.map((c) => c.id))
    const incomingIds = new Set(body.cards.filter((c: any) => c.id).map((c: any) => c.id))

    // Delete removed cards
    for (const ec of existingCards) {
      if (!incomingIds.has(ec.id)) {
        await db.delete(monthlyRecords).where(eq(monthlyRecords.cardId, ec.id))
        await db.delete(cards).where(eq(cards.id, ec.id))
      }
    }

    // Upsert cards
    for (const card of body.cards) {
      if (card.id && existingIds.has(card.id)) {
        // Update existing card
        await db.update(cards).set({
          issuer: card.issuer,
          last4: card.last4,
          openingBalance: card.openingBalance,
          openingMonth: card.openingMonth,
          creditLimit: card.limit,
          aprRegular: card.aprRegular,
          aprPromo: card.aprPromo,
          promoUntil: card.promoUntil,
          dd: card.dd,
          ddAmount: card.ddAmount,
          paymentDay: card.paymentDay,
          statementDay: card.statementDay ?? 1,
          source: card.source ?? "manual",
          tlAccountId: card.tlAccountId ?? null,
          minPaymentOverride: card.minPaymentOverride ?? null,
        }).where(eq(cards.id, card.id))

        // Replace monthly records
        await db.delete(monthlyRecords).where(eq(monthlyRecords.cardId, card.id))
        for (const r of card.monthlyRecords || []) {
          await db.insert(monthlyRecords).values({
            cardId: card.id,
            month: r.month,
            debits: r.debits,
            credits: r.credits,
            interest: r.interest,
            closingBalance: r.closingBalance,
            source: r.source || "manual",
          })
        }
      } else {
        // Insert new card
        const [newCard] = await db.insert(cards).values({
          userId,
          issuer: card.issuer,
          last4: card.last4 || "0000",
          openingBalance: card.openingBalance || 0,
          openingMonth: card.openingMonth || "2025-02",
          creditLimit: card.limit || 0,
          aprRegular: card.aprRegular || 0,
          aprPromo: card.aprPromo,
          promoUntil: card.promoUntil,
          dd: card.dd || "none",
          ddAmount: card.ddAmount || 0,
          paymentDay: card.paymentDay || 5,
          statementDay: card.statementDay || 1,
          source: card.source || "manual",
          tlAccountId: card.tlAccountId || null,
          minPaymentOverride: card.minPaymentOverride ?? null,
        }).returning()

        for (const r of card.monthlyRecords || []) {
          await db.insert(monthlyRecords).values({
            cardId: newCard.id,
            month: r.month,
            debits: r.debits,
            credits: r.credits,
            interest: r.interest,
            closingBalance: r.closingBalance,
            source: r.source || "manual",
          })
        }
      }
    }
  }

  return NextResponse.json({ ok: true })
}
